...
export function useSkyjoGame(): SkyjoGameHook {
  ...
  const joinRoom = useCallback((roomId: string, playerName: string) => {
    ...
    if (saved) {
      ...
      const existingPlayer = savedRoom.players.find(
      ...
      if (updatedRoom.isInitialization) {
        if (player.cardsRevealed >= 2) {
          const nextPlayerIndex =
            (updatedRoom.players.findIndex((p) => p.id === playerId) + 1) %
          ...
          const allReady = updatedRoom.players.every(
  ...
  const exchangeCard = useCallback(
    (playerId: string, row: number, col: number, drawnCard: number) => {
      if (!room) return;
