...
export function useRobustStorage() {
  ...
  const saveGameState = useCallback((roomId: string, playerId: string, playerName: string, gameData: any) => {
  ...
  const loadGameState = useCallback((roomId: string): GameStateData | null => {
    const sources = [
  ...
  const loadPlayerSession = useCallback(() => {
    try {
      ...
      const gameStateCookie = cookies.find(c => c.trim().startsWith('game_state='));
  ...
  // Restore from server
  const restoreFromServer = useCallback(async (roomId: string): Promise<any | null> => {
    try {
